...
//
//   1. `listing_photos.is_cover` did not exist on the sister project. Every
//      query in the ported moderation queue failed; pages rendered 200 and
//      empty. The same class of gap can open here on any schema change.
...
/** The kinds `/admin/paiements` can settle — the v3 product line. */
...
await check("catalogue", "categories tree", () =>
  sb.from("categories").select("id, parent_id, slug, label_fr, sort_order, is_active")
    .order("sort_order"));
