...
export default async function MyPaymentsPage({
  ...
  //
  // Through `metadata.listing_id`, not a foreign key. This block read
  // `auctions`, `properties`, `property_photos` and `auction_deposits`, and the
  // select above named `payments.auction_id` and `payments.property_id`. All
  // four tables have held zero rows since the pivot, and v3 fees never
  // populated either column — they carry their subject in metadata, which is
  // what /api/admin/paiements has always read.
  //
  // So every payment on this page rendered with no title, no location and no
  // thumbnail: a list of bare amounts with no indication of what they were for.
  const listingIds = [
    ...new Set(
      payments
  ...
  // the summary shape: the client renders these tiles conditionally on > 0, so
