...
import { CheckCircle2, Check, Scale } from "lucide-react";

// The platform runs one auction format — the price goes up, the highest
// offer at closing time wins. There is nothing to pick, so the form has
// no format step at all: no picker, no name, no explainer card.
...
export function ScheduleForm({
  ...
  // Legal 1/6 overbid window — opt-in per auction (default off).
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      const payload: Record<string, unknown> = {
        ...
        // The only format the platform runs. The DB guard (migration
        // 0150) rejects anything else, so this is never a choice.
        type: "english",
        ...
        sixth_offer_enabled: sixthOfferEnabled,
      };
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      {/* ── 2. Période ── */}
      <section>
        <SectionTitle n={2} title="Période" />
      ...
      {/* ── 3. Surenchère du 1/6 (opt-in) ── */}
      <section>
      <SectionTitle n={3} title="Surenchère du 1/6 (optionnel)" />
        <button
          type="button"
